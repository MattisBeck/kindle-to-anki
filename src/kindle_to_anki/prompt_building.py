"""Build Gemini prompts from grouped Kindle vocabulary records."""
...
def make_word_block(batch: list[WordRecord]) -> str:
    """Format one batch of vocabulary records for inclusion in a prompt."""
...
def build_shared_prompt():
    """Build prompt instructions shared by all vocabulary card types."""
    prompt = """
...
- anchor: shortest exact word or phrase copied from the context that identifies the relevant usage; usually one word, up to five words for idioms, phrasal verbs, or fixed expressions; this is evidence for meaning, not the reverse-card cloze
...
def get_language(language_code: str) -> str:
    """Return the English language name for a supported language code."""
    try:
...
def build_foreign_vocabulary_prompt(source_language_code:str, native_language_code:str) -> str:
    """Build task instructions for foreign-language vocabulary cards."""
...
- optional cloze_phrase in the source language for reverse cards, only when the learner should recall a whole expression
...
13. For normal single-word vocabulary, leave cloze_phrase empty.
14. Set cloze_phrase only for a real fixed expression, idiom, phrasal verb, or collocation where hiding only the input word would test the wrong thing.
15. If set, cloze_phrase must be copied exactly from the context and must contain the input word exactly.
"""
    return specified_prompt.strip()


def build_definition_prompt(source_language_code: str, native_language_code: str) -> str:
    """Build task instructions for native-language definition cards."""
    source_language = get_language(source_language_code)
    native_language = get_language(native_language_code)
    if source_language_code != native_language_code:
