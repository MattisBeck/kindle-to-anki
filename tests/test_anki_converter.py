...
def test_reverse_model_styles_cloze_context() -> None:
    model = create_anki_model(NATIVE_FOREIGN)

    assert ".context b,\n.context .cloze" in model.css
    assert ".context .cloze" in model.css
    assert "background-color: transparent" in model.css
...
def test_cloze_context_hides_requested_text() -> None:
    context = "Low cloud moved across the ridge."

    clozed_context = cloze_context(context, "cloud")

    assert clozed_context == "Low {{c1::cloud}} moved across the ridge."


def test_cloze_context_leaves_context_when_text_is_missing() -> None:
    context = "Low clouds moved across the ridge."

    clozed_context = cloze_context(context, "rain")

    assert clozed_context == "Low clouds moved across the ridge."
...
def test_prompt_jobs_to_anki_cards_foreign() -> None:
    ...
    assert card.context_html == "<b>Low clouds</b> moved across the ridge."
...
def test_prompt_jobs_to_anki_cards_uses_valid_cloze_phrase_for_reverse_card() -> None:
    book = SourceBook("Broken Orbit", "Evan Shore")
    word = WordRecord("give", "en", "give", "They would give up before dawn.", book)
    batch = ForeignVocabularyBatch.model_validate_json("""
    {
        "items": [
            {
    ...
    reverse_card = cards_by_language_pair["de_en"][0]
    assert reverse_card.context_html == "They would {{c1::give up}} before dawn."


def test_prompt_jobs_to_anki_cards_ignores_invalid_cloze_phrase_for_reverse_card() -> None:
    book = SourceBook("Broken Orbit", "Evan Shore")
    word = WordRecord("give", "en", "give", "They would give up before dawn.", book)
    batch = ForeignVocabularyBatch.model_validate_json("""
    {
        "items": [
            {
                "item_index": 0,
                "lemma": "give",
                "definition": "to transfer or yield",
                "ambiguity": "low",
                "anchor": "give up",
                "confidence": 1.0,
                "gloss": "geben",
                "cloze_phrase": "give in"
            }
        ]
    }
    """)
