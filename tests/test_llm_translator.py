...
def get_foreign_json(cloze_phrase: str | None = None) -> str:
    cloze_phrase_line = "" if cloze_phrase is None else f',\n                "cloze_phrase": "{cloze_phrase}"'
    return """
    {
        "items": [
            {
...
def test_parse_response_native_definition() -> None:
    ...
    assert not hasattr(parsed_response.items[0], "cloze_phrase")
...
def test_parse_response_foreign_vocabulary() -> None:
    ...
    assert parsed_response.items[0].cloze_phrase == ""
...
def test_validate_response_matches_job_keeps_valid_cloze_phrase() -> None:
    response = ForeignVocabularyBatch.model_validate_json(get_foreign_json("The house"))
    prompt_job = PromptJob(
        "",
        PromptType.FOREIGN_VOCABULARY,
        [WordRecord("house", "en", "house", "The house is old.", SourceBook("Book", "Author"))],
        "de",
        "en",
    )

    assert validate_response_matches_job(response, prompt_job)
    assert response.items[0].cloze_phrase == "The house"


def test_validate_response_matches_job_clears_invalid_cloze_phrase() -> None:
    response = ForeignVocabularyBatch.model_validate_json(get_foreign_json("The roof"))
    prompt_job = PromptJob(
        "",
        PromptType.FOREIGN_VOCABULARY,
        [WordRecord("house", "en", "house", "The house is old.", SourceBook("Book", "Author"))],
    ...
    assert validate_response_matches_job(response, prompt_job)
    assert response.items[0].cloze_phrase == ""
